...
Complete rewrite from bash scripts to a Go binary with Board integration.
...
- **Board-based completion verification** - Checks Board task stats instead of Haiku LLM verification
  ...
  - Automatic feedback to Board when agent falsely claims completion
- **`ralph run --plan <id>`** - Run iteration loop on a specific Board plan
  - `--completion-mode` flag: pr, merge, or branch
  - `--push` flag: push to remote after each iteration
- **`ralph worker`** - Continuous queue processor polling Board for ready plans
...
- **`ralph init`** - Interactive project initialization
  ...
  - Board configuration prompts (project slug, API URL, API token)
  - Specs directory scaffolding
- **`ralph status`** - Board project status display
...
- **Configurable Board** - `board:` config section (project_slug, api_url, api_token, bin_path)
- **Worker config** - `worker:` config section (sync, sync_interval)
- **Fake Board test infrastructure** - `test/integration/fakeboard/` binary for isolated testing
...
- **Task management**: Board API replaces filesystem-based plan queue (plans/pending, current, complete)
- **Completion verification**: Board task stats replace Haiku LLM verification
...
- **Haiku verification**: `internal/runner/verify.go` (replaced by Board stats checking)
- **Plan bundles**: `ralph plan create` command and bundle scaffolding
- **`ralph reset`** command (use Board to change plan status)
