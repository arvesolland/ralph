...
// TestBuildPRBody was removed - PR body is now generated by Claude
...
// TestCreatePR_Fallback tests the fallback PR creation with a mock gh script
func TestCreatePR_Fallback(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test in short mode")
	}
...
`
	...
	// Run the fallback PR creation (with our mock gh)
	prURL, err := createPR(p, tmpDir)
	if err != nil {
		t.Errorf("createPR() error = %v", err)
	}

	if prURL != "https://github.com/test/repo/pull/123" {
		t.Errorf("createPR() prURL = %q, want %q", prURL, "https://github.com/test/repo/pull/123")
	}
}
