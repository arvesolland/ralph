...
- If it says to skip a task, run `board-cli task skip <task-id> --reason "per feedback: ..."`.
...
```bash
board-cli plan context {{PLAN_ID}} --format text
```
...
**Blocked tasks:** If you cannot complete a task, use `board-cli task skip <task-id> --reason "..."` or `board-cli task block <task-id> --reason "..."`.
...
**board-cli errors:** If any `board-cli` command fails, note the error and continue with your implementation. Use `board-cli progress add {{PLAN_ID}} --author ralph --body "..."` to log issues.
