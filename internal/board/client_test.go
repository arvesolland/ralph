...
func TestNewClientDefaults(t *testing.T) {
	...
	if c.binPath != "board" {
		t.Errorf("binPath = %q, want %q", c.binPath, "board")
	}
}
...
func TestExecSuccess(t *testing.T) {
	...
	dir := t.TempDir()
	bin := writeFakeScript(t, dir, "board", `echo '{"data":{"id":1,"title":"Test Plan","status":"active"}}'`)
...
func TestExecError(t *testing.T) {
	...
	dir := t.TempDir()
	bin := writeFakeScript(t, dir, "board", `echo "not found: plan 999" >&2; exit 1`)
...
func TestExecTimeoutErrorFormat(t *testing.T) {
	...
	//   "board <subcommand>: timed out after <duration>"
	...
	bin := writeFakeScript(t, dir, "board", `kill -TERM $$`)
...
func TestExecParsesGlobalFlags(t *testing.T) {
	...
	bin := writeFakeScript(t, dir, "board", `
...
func TestPlanContextSuccess(t *testing.T) {
	...
	dir := t.TempDir()
	bin := writeFakeScript(t, dir, "board", `echo '{"data":{"project":{"id":1,"slug":"test"},"plan":{"id":42,"title":"Test","status":"active"},"stats":{"total_tasks":3,"done":1,"doing":0,"claimed":0,"blocked":0,"available":2,"skipped":0},"available_tasks":[],"blocked_tasks":[],"recent_progress":[],"recent_feedback":[]}}'`)
...
func TestPlanContextTextSuccess(t *testing.T) {
	...
	dir := t.TempDir()
	bin := writeFakeScript(t, dir, "board", `echo "Plan #42: Test Plan\nStatus: active"`)
...
func TestCompleteTaskSuccess(t *testing.T) {
	...
	dir := t.TempDir()
	bin := writeFakeScript(t, dir, "board", `echo '{"data":{"id":10,"plan_id":42,"title":"Task","status":"done"}}'`)
...
func TestAddProgressSuccess(t *testing.T) {
	...
	dir := t.TempDir()
	bin := writeFakeScript(t, dir, "board", `echo '{"data":{"id":1,"plan_id":42,"author":"ralph","body":"iteration done"}}'`)
